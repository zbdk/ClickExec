...
7. WHEN Tool_Definition が0件または `clickExec.tools` 設定キーが未定義の場合、THE Extension SHALL ユーザーに確認ダイアログを表示し、「はい」を選択した場合はデフォルトツールを settings.json のグローバル設定に書き込む。「いいえ」を選択した場合またはダイアログを閉じた場合は、メモリ上のデフォルトツールをフォールバックとして使用する
